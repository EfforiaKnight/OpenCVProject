...
class MouseGUI():
    ...
    def initialize(self):
        ...
        while True:
            ...
            for i in range(sizeVec):
                cv2.line(img, self.mouseV[i], self.mouseV[i + 1], (255, 255, 0), 1)
            ...
            if int(self.counter) == int(0):
                ...
                for z in range(self.model.SamplesNum):
                    ...
                    self.model.flConfidence[z] = 1.0 / (np.sqrt(np.power(diffX, 2) + np.power(diffY, 2)))
    ...
    def update_after_iterating(self, img):
        ...
        sizeVec = len(self.mouseV) - 1
        print("Mean: {} {}".format(meanInt[0], meanInt[1]))
        print("Real: {} {}".format(self.mouseV[sizeVec][0], self.mouseV[sizeVec][1]))
        print('+++++++++++++++')
